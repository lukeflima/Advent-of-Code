...
std::string part1(const std::string_view input) {
    ...
    auto input_cb = [&]() -> long long {
        ...
        if(index < spring_script.size()) {
            return (long long) spring_script[index++];
        }
        return 0;
    };
    long long damage_reported = 0;
    auto output_cb = [&](long long output) {
        if(output > 255) damage_reported = output;
    };

    intcode_computer(input, input_cb, output_cb);

    return std::to_string(damage_reported);
}
